    ...
    public boolean updateClient(int cCode, String csvClient) {
        ...
        for (Client aux : data.getClients()) {
            if (aux.getClientCode() == cCode) {
                if (c.getName() != null && !c.getName().isBlank()) aux.setName(c.getName());
                if (c.getSurname1() != null && !c.getSurname1().isBlank()) aux.setSurname1(c.getSurname1());
                if (c.getSurname2() != null && !c.getSurname2().isBlank()) aux.setSurname2(c.getSurname2());
                if (c.getNif() != null && !c.getNif().isBlank()) aux.setNif(c.getNif());
                if (c.getEmail() != null && !c.getEmail().isBlank()) aux.setEmail(c.getEmail());
                if (c.getTelephone() != null && !c.getTelephone().isBlank()) aux.setTelephone(c.getTelephone());
                return true;
            }
        }
        return false;
...
//    }
    ...
    public boolean deleteClientByNif(String nif) {
        ...
        Client clientToRemove = data.getClients().stream().filter(c -> c.getNif().equalsIgnoreCase(nif)).findFirst().orElse(null);

        if (clientToRemove == null) return false;
